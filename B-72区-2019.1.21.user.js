...
function createButton(a, b, c) {
    ...
    var d = btnList[a];
    d.innerText = a;
    d.style.marginTop = currentPos1 + "px";
    d.style.padding = '0px';
    d.style.width = '100%';
...
buttonhideAllButton2.style.top = '440px';
...
buttonhideAllButton.style.top = '420px';
currentPos = currentPos + delta;
...
var mySkillLists = "八荒功;九溪断月枪;排云掌法;如来神掌;九天龙吟剑法;燎原百破;昊云破周斧;千影百伤棍;破军棍决;天火飞锤;织冰剑法;无影毒阵;唐门毒功;";
...
//-----自动绝学阵变量
var skillstr = "排云掌法,燎原百破,6,道种心魔经";
var skillstr6 = "排云掌法,燎原百破,6,道种心魔经";
var skillstr9 ="千影百伤棍,燎原百破,9,道种心魔经";
...
createButton('放毒',btnBox0,PaiHangFunc1);
//隐藏所有按钮的按钮----------------------------------
var buttonhiden=0;
...
function WhoAmIFunc() {
    ...
    if (llmyattrs){
        ...
        if (myID == 'u5885237'){   // 青天
            skillstr6 = "燎原百破,九溪断月枪,6,道种心魔经";
            skillstr9 = "千影百伤棍,燎原百破,6,道种心魔经";
             mySkillLists = "燎原百破,九溪断月枪";
            //autoBattleFunc();
            addXueFunc();
        }
       if (myID == 'u6682109'){   //宇文
        ...
        if (myID == 'u6684925'){   // 门哥
         ...
         if (myID == 'u2947893'){   // 贪狼
            skillstr6 = "昊云破周斧,昊云破周斧,6,道种心魔经";
            skillstr9 = "破军棍诀,昊云破周斧,6,道种心魔经";
            addXueFunc();
        }
      /*  if (myID == 'u7630388'){   // 烦
...
//雪婷--------------------------------------------------
createPopButton('铁血求教','直通车',xuetingzhenFunc);
function xuetingzhenFunc(){
    go("jh 31;n;se;e;se;s;s;sw;se;se;e;nw;e;ne;n;ne;n;n;n;n;n;n;");
}

//洛阳--------------------------------------------------
createPopButton('木人','直通车',lyFunc);
function lyFunc(){    go("jh 41;se;e;e;se;se;se;se;se;se;event_1_57976870;n;n;n;event_1_91914705;"); }
...
//createPopButton('华山换正气','外传日常',GQFunc);
...
function ZhuangBei(){
    ...
    { console.log("切换战斗装备！");
     ...
     go('unwield longwulianmoge_xiefoyaochui');//脱
     go('unwield longwulianmoge_yuyemoqiang');//脱
     go('unwield weapon_sb_unarmed11');       // 脱拳套
     go('unwield weapon_sb_spear12');       // 脱枪
     go('wield weapon_sb_spear12');        //穿龙吟剑
...
function ZhuangBei2(){
    if(btnList["战斗装"].innerText == '战斗装'){
        ...
        go('wield weapon_sb_unarmed11');       // 穿拳套
        btnList["战斗装"].innerText = '打坐装';
    }
    else{
        console.log("切换打坐装备！");
...
function GQFunc(){
    //   alert("VIP专用\n\n请手动完成最后一个任务");
    var trueBeToDo=confirm("刷花钱正气哦,钱包够不够鼓？");
    if (!trueBeToDo) {
        return;
         }
   // go("home;clan;clan scene;clan task;home;");
...
// 宽宽施毒----------------------------
function PaiHangFunc1(){
    if(btnList["放毒"].innerText  == '放毒'){
        btnList["放毒"].innerText  = '停止放毒';
        //go('home');
        //go('sort');
        go('fight snow_dukuankuan');
        setTimeout(AutoPaiHangFunc1,3000);//code
    }
}
function AutoPaiHangFunc1(){
    if( isContains($('span:contains(今日挑战)').text().slice(-19), '今日挑战高手的次数已达上限，明日再来。')){
        paustStatus=0;
        btnList["放毒"].innerText  = '放毒';
        console.log('打完收工！');
    }
    else{
        go('fight snow_dukuankuan');
...
function KuafuBossFunc() {
  //  var realmInfo=g_obj_map.get("msg_room").get("map_id");
    if (kuafuBossFlag==0){
        kuafuBossFlag=1;
        mySkillLists = "千影百伤棍,八荒功,破军棍诀";
        btnList1['换群伤'].innerText = '停群伤';
    //    var locationname=g_obj_map.get("msg_room").get("short");
    //    console.log(locationname);
       // KuafuBossRun();
       ...
       // kuafuBossInterval = setInterval(function(){ clickButton('kill kuafu_wushuanggongzhu');clickButton('kill kuafu_buerjianke');}, 1000);
        ...
        clearInterval(kuafuBossInterval);
        kuafuBossFlag=0;
        mySkillLists = "燎原百破，四海断潮斩";
        btnList1['换群伤'].innerText = '换群伤';
    }
}
//==========================================================
    ...
    this.dispatchMessage = function(b) {
        ...
        if (type=="main_msg"){  //handle main message;
            ...
            if (( msg.match("71-75区]")!=null) && (KFQLtrigger == 1)){
                //监控 71-75  青龙(周一、周二）-
                if (( msg.match("晚香玉")!=null || msg.match("凌霄花")!=null || msg.match("百宜雪梅")!=null || msg.match("朝开暮落花")!=null || msg.match("凤凰木")!=null || msg.match("熙颜花")!=null ||msg.match("夕雾草")!=null ||  msg.match("彼岸花")!=null || msg.match("洛神花")!=null || msg.match("碎片")!=null || PTtrigger == 1) && ( msg.match("荣威镖局")==null)){ // pt triggsuer = 1 是默认， =0 时 打所有跨服青龙
//                  if (( msg.match("晚香玉")!=null || msg.match("凌霄花")!=null || msg.match("百宜雪梅")!=null || msg.match("朝开暮落花")!=null || msg.match("凤凰木")!=null || msg.match("熙颜花")!=null || msg.match("君影草")!=null ||msg.match("矢车菊")!=null ||msg.match("忘忧草")!=null ||msg.match("仙客来")!=null ||msg.match("雪英")!=null ||msg.match("夕雾草")!=null ||  msg.match("彼岸花")!=null || msg.match("洛神花")!=null || PTtrigger == 0) && ( msg.match("荣威镖局")==null)){ // pt triggsuer = 1 是默认， =0 时 打所有跨服青龙
            ...
            if (QLtrigger==1){
                if (msg.match("青龙会组织")!=null){
                    //                    console.log(msg);
                    if((msg.match("晚香玉")!=null || msg.match("凌霄花")!=null || msg.match("百宜雪梅")!=null || msg.match("朝开暮落花")!=null || msg.match("凤凰木")!=null || msg.match("熙颜花")!=null || msg.match("君影草")!=null ||msg.match("矢车菊")!=null ||msg.match("忘忧草")!=null ||msg.match("仙客来")!=null ||msg.match("雪英")!=null ||msg.match("夕雾草")!=null ||  msg.match("彼岸花")!=null || msg.match("洛神花")!=null || msg.match("碎片")!=null || msg.match("岳老大")!=null) || ((msg.match("碎片")!=null)&&(msg.match("轩辕剑碎片")==null)&&(msg.match("破岳拳套碎片")==null)) || (msg.match("乾坤再造丹")!=null) || (msg.match("灵草")!=null) || (msg.match("小还丹")!=null) || (msg.match("紫芝")!=null) || (msg.match("狂暴丹")!=null)){
                        var url=msg.split("href;0;")[1].split("")[0];
                        tarNPC=msg.split("组织：")[1].split("正在")[0];
                        clickButton(url);
                        Qinglong();
...
function addXue(){
    ...
    if(kee/max_kee <= 2/3 || force<10000) {
        //设置自动回血回蓝目标值 1/3  三分子一血量自动回血， 小于1w 内不动回内
        //战斗中？
...
//var buttonWidth = '70px';   // 按钮宽度
//var buttonHeight = '22px';  // 按钮高度
var currentPos1 = 0;// 当前按钮距离顶端高度，初始130
//var delta = 20;                 // 每个按钮间隔
//按钮加入窗体中----------------------------
...
createButton1('换群伤',KuafuBossFunc);
...
function hideButton1(){
    ...
    btnList1['换群伤'].style.visibility="hidden";
...
function showButton1(){
    ...
    btnList1['换群伤'].style.visibility="visible";
...
var items_use='兰陵美酒 周年英雄令 周年热血令 神鸢宝箱 茉莉汤 云梦青 『神匠宝箱』冰糖葫芦『秘籍木盒』青凤纹绶 热血印 风云宝箱 腊八粥 腊百草美酒元宵 年糕 高级狂暴丹特级狂暴丹保险卡特级大还丹高级大还丹小还丹百年紫芝百年灵草特级乾坤再造丹高级乾坤再造丹神秘宝箱冰镇酸梅汤';
var items_store='狗年礼券曜玉宝箱空识卷轴长生石玫瑰花武穆遗书晚香玉 凌霄花 百宜雪梅 朝开暮落花 凤凰木 熙颜花 君影草 矢车菊 忘忧草 仙客来 雪英 夕雾草 彼岸花 洛神花 青木宝箱 千年紫芝千年灵草驻颜丹烧香符周年礼券玄重铁分身卡鱼竿鱼饵江湖令谜题令正邪令状元贴白银宝箱黄金宝箱铂金宝箱高级乾坤袋装备打折卡碎片黄金钥匙鎏金黑玉锥曜玉钥匙铂金钥匙赤璃钥匙';
var items_study=' 左手兵刃研习';
var items_splite='星河剑 翎眼赤护 青鸾护臂 苍狼护臂 宝玉甲 天寒匕 貂皮斗篷 白玉腰束 无心匕 玄武盾 月光宝甲 沧海护腰 夜行披风虎皮腰带红光匕金丝甲羊毛斗篷破军盾金丝甲疯魔杖毒龙鞭玉清棍生死符霹雳掌套血屠刀残雪帽残雪戒残雪鞋残雪手镯残雪项链金丝宝甲衣';
...
// 杀坏人----------------------------------------------------------------------------------------------------------------
var HongMingNPCList =["[71-75区]段老大","[71-75区]墟归一","[71-75区]上官晓芙","[71-75区]洪昭天","恶棍", "流寇", "剧盗","云老四", "岳老三", "二娘","段老大","黑袍公","独孤须臾","翼国公","云观海","年兽","铁狼军","银狼军","金狼军","金狼将","十夫长","百夫长","月幽剑士","濯缨剑士","对影剑士","夏花剑士","[一]镇擂斧将","[二]镇擂斧将","天魔真身","攻楼死士","镇山神兽","守山神兽","应龙幼崽","应龙兽魂","应龙分身","应龙王","应龙战神","镇潭神兽","守潭神兽","螣蛇幼崽","螣蛇兽魂","螣蛇分身","螣蛇王","螣蛇战神","镇殿神兽","守殿神兽","幽荧幼崽","幽荧兽魂","幽荧分身","幽荧王","幽荧战神","饕餮分身","饕餮兽魂","饕餮幼崽","守谷神兽","镇谷神兽","饕餮王","饕餮战神","不『二』剑客"];
